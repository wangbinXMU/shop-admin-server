...
router.get(
  ...
  function (req, res, next) {
    ...
    catServ.getAllCategories(req.query.type, conditions, function (
      err,
      result
    ) {
...
router.put(
  ...
  // 业务逻辑
  function (req, res, next) {
    catServ.updateCategory(req.params.id, req.body.cat_name, function (
      err,
      result
    ) {
      if (err) return res.sendResult(null, 400, err);
...
router.get(
  ...
  function (req, res, next) {
    // attrServ
    attrServ.getAttributes(req.params.id, req.query.sel, function (
      err,
      attributes
    ) {
      if (err) return res.sendResult(null, 400, err);
